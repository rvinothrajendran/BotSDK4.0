...
namespace MeetupDemo
{
    ...
    public partial class MeetupDemoBot : IBot
    {
        private readonly StateAccessor _stateAccessor;
        /// <summary>
        /// Initializes a new instance of the class.
        /// </summary>                        
        public MeetupDemoBot(StateAccessor stateAccessor)
        {
            _stateAccessor = stateAccessor;
            PrepareDialogSet(_stateAccessor.DlgState);
        }
        ...
        public async Task OnTurnAsync(ITurnContext turnContext, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                DialogContext dlgContext = await _dialogSet.CreateContextAsync(turnContext, cancellationToken);

                if (dlgContext?.ActiveDialog == null)
                {
                    if (dlgContext != null)
                        await dlgContext.BeginDialogAsync("mainDlg", cancellationToken: cancellationToken);
                }
                else
                {
